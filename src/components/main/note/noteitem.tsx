...
export default function NoteItem({ item }: any) {
   ...
   const delNoteMutation = useMutation({
      ...
      async onSuccess(data) {
         console.log(data);
         await queryClient.invalidateQueries({ queryKey: ['fetchNotes', 'NOTE'] });
      },
      onError(err) {
         console.log(err);
      },
   ...
   return (
      ...
      >
         ...
         >
            ...
            <div onClick={() => setstate(!state)} className='flex flex-row items-start gap-3 w-full p-3 cursor-pointer'>
